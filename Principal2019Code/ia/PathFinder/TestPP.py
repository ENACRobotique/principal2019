...
class PurePursuit:
    def __init__(self, cx, cy, current_robot):
        ...
        self.cy = cy
        SPEED_MAX = 300
        self.target_speed = SPEED_MAX/(100*3.6)
        ...
        self.Save_x = [self.state.x]
        self.Save_y = [self.state.y]
        self.Save_yaw = [self.state.yaw]
        self.Save_v = [self.state.v]
    ...
    def SaveLog(self) : 
        ...
        self.Save_y.append([self.state.y])
...
def main():
    ...
    # initial state
    state = State(x=0.0, y=0.0, yaw=-math.pi/2, v=0.0)
